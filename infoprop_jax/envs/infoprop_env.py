...
class InfopropEnv(Wrapper):
  ...
  Whether the wrapped env builds an MJX ``pipeline_state`` during model rollouts
  (the "fast rollout" optimisation) is entirely the wrapped env's concern — this
  wrapper is agnostic to it and never reads or sets such a flag.
  """
